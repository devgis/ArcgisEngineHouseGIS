    ...
    public sealed partial class MainForm : Form
    {
        ...
        private void CreateHouseLayer()
        {
            ...
            try
            {
                ...
                FieldClass feadHouseUser = new FieldClass();
                IFieldEdit edit = feadHouseUser;
                edit.Name_2 = "HouseUser";
                edit.Type_2 = esriFieldType.esriFieldTypeString;
                oFieldsEdit.AddField(edit);

                //创建图元属性
                FieldClass feadID = new FieldClass();
                IFieldEdit editID = feadID;
                editID.Name_2 = "HouseID";
                editID.Type_2 = esriFieldType.esriFieldTypeString;
                oFieldsEdit.AddField(editID);

                

                FieldClass feadHouseArea = new FieldClass();
                IFieldEdit editHouseArea = feadHouseArea;
                editHouseArea.Name_2 = "HouseArea";
                editHouseArea.Type_2 = esriFieldType.esriFieldTypeString;
                oFieldsEdit.AddField(editHouseArea);

                FieldClass feadHouseAddress = new FieldClass();
                IFieldEdit editHouseAddress = feadHouseAddress;
                editHouseAddress.Name_2 = "HouseAddress";
                editHouseAddress.Type_2 = esriFieldType.esriFieldTypeString;
                oFieldsEdit.AddField(feadHouseAddress);
        ...
        {
            DataTable dt = DBHelper.Instance.GetDataTable("select * from House");
            foreach (DataRow dr in dt.Rows)
            {
                string HouseID = dr["HouseID"].ToString();
                string HouseArea = dr["HouseArea"].ToString();
                string HouseAddress = dr["HouseAddress"].ToString();
                string HouseUser = dr["HouseUser"].ToString();
                ...
                int field1 = pFeature.Fields.FindField("HouseID");
                pFeature.set_Value(field1, HouseID??"");
                field1 = pFeature.Fields.FindField("HouseArea");
                pFeature.set_Value(field1, HouseArea ?? "");
                field1 = pFeature.Fields.FindField("HouseAddress");
                pFeature.set_Value(field1, HouseAddress ?? "");
                field1 = pFeature.Fields.FindField("HouseUser");
                pFeature.set_Value(field1, HouseUser ?? "");
        ...
        private void UniqueValueRenderFlyr(IFeatureLayer pFeatureLayer)
        {
            ...
            pUVRender.DefaultLabel = "[HouseUser]";
...
//pBasicOverposeterLayerProp.PointPlacementMethod = esriOverposterPointPlacementMethod.esriSpecifiedAngles;
        ...
        private void cmiAddEquip_Click(object sender, EventArgs e)
        {
            ...
            if (frmAddHouse.ShowDialog() == DialogResult.OK)
            {

                IPoint point = axMapControl1.ActiveView.ScreenDisplay.DisplayTransformation.ToMapPoint(PosX, PosY); ;

                string HouseID = Guid.NewGuid().ToString();
                string HouseUser = frmAddHouse.HouseUser;
                string HouseArea = frmAddHouse.HouseArea.ToString();
                string HouseAddress = frmAddHouse.HouseAddress;


                string sql = string.Format("insert into House (HouseID,HouseUser,HouseAddress,HouseArea,PosX,PosY) values('{0}','{1}','{2}','{3}',{4},{5})"
                    , HouseID, HouseUser, HouseAddress, HouseArea, point.X, point.Y);
        ...
        private void cmiSearch_Click(object sender, EventArgs e)
        {
            ...
            if (searchForm.DialogResult == DialogResult.OK)
            {
                ...
                queryFilter.WhereClause = String.Format("HouseUser like '%{0}%' or HouseAddress like '%{0}%'", searchForm.Name);
        ...
        private void SelectFeature(int x, int y)
        {
            ...
            IFeatureLayer pFeatureLayer = EquipLayer as IFeatureLayer;
            ...
            if (pFeature != null)
            {
                ...
                int fieldHouseIDIndex = pFeature.Fields.FindField("HouseID");
                string HouseID = pFeature.get_Value(fieldHouseIDIndex).ToString();
                int fieldHouseAreaIndex = pFeature.Fields.FindField("HouseArea");
                string HouseArea = pFeature.get_Value(fieldHouseAreaIndex).ToString();
                int fieldHouseAddressIndex = pFeature.Fields.FindField("HouseAddress");
                string HouseAddress = pFeature.get_Value(fieldHouseAddressIndex).ToString();
                int fieldHouseUserIndex = pFeature.Fields.FindField("HouseUser");
                string HouseUser = pFeature.get_Value(fieldHouseUserIndex).ToString();
                AddHouse frmAddHouse = new AddHouse();
                frmAddHouse.HouseAddress = HouseAddress;
                try
                {
                    frmAddHouse.HouseArea = Convert.ToDouble(HouseArea);
                }
                catch
                {
                    frmAddHouse.HouseArea = 0;
                }
                frmAddHouse.HouseUser = HouseUser;
                frmAddHouse.Text = "编辑房屋信息";
                if (frmAddHouse.ShowDialog() == DialogResult.OK)
                {
                    string sql = string.Format("update House set HouseUser='{0}',HouseAddress='{1}',HouseArea='{2}' where HouseID='{3}'", frmAddHouse.HouseUser, frmAddHouse.HouseAddress, frmAddHouse.HouseArea, HouseID);
                    if (DBHelper.Instance.ExcuteSql(sql))
                    {
                        ...
                        ////////pWorkspaceEdit.StartEditing(false);
                        //pWorkspaceEdit.StartEditOperation();
                        int field1 = pFeature.Fields.FindField("HouseUser");
                        pFeature.set_Value(field1, frmAddHouse.HouseUser);
                        field1 = pFeature.Fields.FindField("HouseArea");
                        pFeature.set_Value(field1, frmAddHouse.HouseArea);
                        field1 = pFeature.Fields.FindField("HouseAddress");
                        pFeature.set_Value(field1, frmAddHouse.HouseAddress);
                        //pFeature.Store();
                        //try
                        //{
                        //    pFeatureCuror.UpdateFeature(pFeature);
                        //}
                        //catch (Exception ex)
                        //{ }
                        //pFeature.Store();
